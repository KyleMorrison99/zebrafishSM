...
pdo <- pdo %>%
  ...
  mutate(dosage_lowest = as.numeric(dosage_lowest),
         ...
         dosage_lowest_convert_ugL = case_when(
           ...
           dosage_lowest_unit == "ppb" ~ dosage_lowest * 1000, # 1 ppb = 1 ug/L
           dosage_lowest_unit == "ppm" ~ dosage_lowest/1000, # 1 ppm = 1 mg/L
           TRUE ~ dosage_lowest),
...
pdo_duration <- pdo %>% 
  ...
  duration_convert = case_when(
    duration_unit == "minutes" ~ duration* 60, # if the original unit is minute, it should be /60
