...
fd <- separate_rows(fd, life_stage_exposure, sep = ",", convert = TRUE)
fd <- separate_rows(fd, pesticide_target_class, sep = ",", convert = TRUE)
...
ggplot(fd_summary, aes(x = pesticide_target_class, y = life_stage_exposure, fill = count)) +
...
```{r}
fd <- separate_rows(fd, behavioural_class, sep = ",", convert = TRUE)
fd <- separate_rows(fd, pesticide_chemical_class, sep = ",", convert = TRUE)
...
# Create a heatmap using ggplot2
ggplot(fd_summary_top5, aes(x = pesticide_chemical_class, y = behavioural_class, fill = count)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "#BFEFFF", high = "#0047AB") +
