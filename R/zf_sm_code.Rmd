...
fig3a <-  ggplot(pesticide_pct, aes(x = reorder(pesticide_investigated, n), y = percentage)) +
 ...
 geom_text(data = pesticide_pct, aes(label = paste0(round(percentage), "%")), 
 ...
 fig3a
#ggsave(here("figures", "fig3a_pesticide_count.pdf"), width = 8, height = 10, units = "cm", scale = 2, dpi = 300, device = cairo_pdf)
```
...
fig3b <- ggplot(target_class_pct, aes(reorder(pesticide_target_class, n), y = percentage)) +
  ...
  # Add labels to the bars for percentage 
 geom_text(data = target_class_pct, aes(label = paste0(round(percentage), "%")), 
            position = position_dodge(width = 0.9), hjust = -0.2, size = 5, color = "black", fontface = "bold") +
 ...
 fig3b
#ggsave(here("figures", "fig3b_pesticide_target_count.pdf"), width = 8, height = 10, units = "cm", scale = 2, dpi = 300, device = cairo_pdf)
```
...
# Calculate target class count as a percentage 
 ...
 fig3c <-  ggplot(chemical_class_pct, aes(x = reorder(pesticide_chemical_class,n), y = percentage)) +
  ...
  # Add labels to the bars for percentage 
 geom_text(data = chemical_class_pct, aes(label = paste0(round(percentage), "%")), 
            position = position_dodge(width = 0.9), hjust = -0.2, size = 5, color = "black", fontface = "bold") +
 ...
 fig3c
#ggsave(here("figures", "fig3c_pesticide_chemical_count.pdf"), width = 8, height = 10, units = "cm", scale = 2, dpi = 300, device = cairo_pdf
```
...
# fig3
#ggsave(here("figures", "fig3_pesticide_count_combined.pdf"), width = 8, height = 10, units = "cm", scale = 2, dpi = 300, device = cairo_pdf)
```
...
figs3
#ggsave(here("figures", "figs3_pesticide_chemical_count.pdf"), width = 8, height = 10, units = "cm", scale = 2, dpi = 300, device = cairo_pdf
```
...
fig4 <- ggplot(pdo_waterbourne, aes(x = dosage_type, y = log(dosage_value))) +
 ...
 fig4
#ggsave(here("figures", "fig4_pesticide_dosage.pdf"), width = 8, height = 10, units = "cm", scale = 2, dpi = 300, device = cairo_pdf)
```
...
figs11 <-  ggplot(behav_activity_pct, aes(x = reorder(behaviour_activity, n), y = percentage)) +
...
figs12 <-  ggplot(behav_aggression_pct, aes(x = reorder(behaviour_aggression, n), y = percentage)) +
...
figs13 <-  ggplot(behav_sociality_pct, aes(x = reorder(behaviour_sociality, n), y = percentage)) +
...
figs14 <-  ggplot(behav_foraging_pct, aes(x = reorder(behaviour_foraging, n), y = percentage)) +
...
figs15 <- ggplot(behav_antipredator_pct, aes(x = reorder(behaviour_antipredator, n), y = percentage)) +
...
figs16 <- ggplot(behav_anxiety_pct, aes(x = reorder(behaviour_anxiety, n), y = percentage)) +
...
figs18 <- ggplot(bd_pd_summary_top3, aes(x = pesticide_chemical_class, y = behavioural_class, fill = count)) +
...
figs20 <- ggplot(sd_pd_summary2, aes(x = pesticide_target_class, y = life_stage_exposure, fill = count)) +
...
figs27 <- fullmap %>% 
  ...
  scale_fill_gradient(low = "#7BCCC4", high = "#084081", # set color gradient
                    name = "Score", na.value = "gray50",
                    limits = c(0.1, 25),
                    guide = guide_colorbar(direction = "vertical.")) +
                    guides(fill = guide_colourbar(barwidth = unit(15, units = "mm"), 
                    barheight = unit(20, units = "mm"))) # set legend size
