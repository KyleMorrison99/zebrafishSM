...
## fig 1- study time time trends
```{r}

fig1 <- bib %>% 
  count(publication_year) %>%
  ggplot(aes(x = publication_year, y = n)) +
   geom_bar(stat = "identity", fill = "#4169E1", color = "white", alpha = 0.8, position = position_dodge(0.9)) +
  geom_text(aes(label = n), position = position_stack(vjust = 0.5), color = "white", size = 4, hjust = 0.5) +
  theme_minimal() +
...
## fig2a - plot for total of individual pesticides used in pesticide exposure studies
```{r}
fig2a <- pd %>%
  ...
  theme(panel.grid.major.y = element_blank(),
        ...
        axis.text.y = element_text(size = 11, hjust = 0.5, angle = 0),
...
## fig2b - total for each pesticide target class used in pesticide exposure studies
```{r}
fig2b <- pd %>%
  separate_rows(pesticide_target_class, sep = ",\\s*") %>%  
  count(pesticide_target_class) %>%
  ggplot(aes(x = n, y = reorder(pesticide_target_class, n))) +
geom_bar(stat = "identity", fill = "#4169E1", color = "white", alpha = 0.8, position = position_dodge(0.9)) +  geom_text(aes(label = n), position = position_stack(vjust = 0.5), color = "white", fontface = "bold", size = 3) +
...
## fig2c - total for chemical class 
```{r} 
fig2c <- pd %>%
...
## fig2d - total route of exposure completed as a percentage
```{r}
...
fig2d <- ggplot(route_pct, aes(x = reorder(route, percentage), y = percentage)) +
  geom_bar(stat = "identity", fill = "#4169E1", color = "white", alpha = 0.8, position = position_dodge(0.9)) +
  geom_text(aes(label = paste0(round(percentage, 1), "%")), hjust = -0.2, vjust = 0.5, size = 3.5, fontface = "bold") +
...
##fig2 
```{r}
fig2 <- ((fig2a)  | (fig2b / fig2c)  + plot_annotation(tag_levels = "A")) # note that we filter  2c for those greater than 1
fig2
fig2d 
```
...
## fig2e - comparison of lowest and highest dosages in waterbourne exposure methods
```{r }
pdo_waterbourne <- pdo %>%
...
## fig2f - comparison of lowest and highest dosages of waterbourne exposure to deltamethrin
```{r}
...
## fig2g - comparison of lowest and highest dosages of waterbourne exposure to rotenone
```{r}
pdo_waterbourne_rotenone<- pdo %>%
  pivot_longer(cols = c(dosage_lowest, dosage_highest), 
               names_to = "dosage_type",
               values_to = "dosage_value") %>% 
  filter(route == "waterbourne", dosage_unit_consistent == "ug/L", pesticide_investigated == "rotenone") %>%  # we had to filter for those studies that gave concentration per litre
  mutate(dosage_type = if_else(dosage_type == "dosage_lowest", "Lowest Dosage Exposed", "Highest Dosage Exposed"))
...
## fig2h - comparison of lowest and highest dosages of waterbourne exposure atrazine
...
## fig2i - comparison to lowest and highest dosages of waterbourne exposure to glyphosate
```{r}
...
## fig 2j - comparison to lowest and highest dosages of waterbourne exposure to chlropyrifo
```{r}
...
## fig2k - total lifestage of exposure completed as a percentage
```{r}
...
## fig 2l - total sexes exposed completed as a percentage
```{r}
# Calculate total count for each category
...
```{r}
fig2.1 <- ((fig2k) /(fig2l) + plot_annotation(tag_levels = "A"))
fig2.1
```

# Objective 2.	To identify the specific behaviours that have been investigated in pesticide exposure experiments that use zebrafish as a model.

## fig3a - total number of studies investigating each broad behaviour class
```{r}
fig3a <- bd %>%
...
## fig3b - total number of studies investigating each behavioural assay of behaviour activity
```{r}
fig3b <- bd %>%
...
## fig3c - total number of studies investigating each behavioural assay of behaviour_aggression
```{r}
fig3c <- bd %>%
...
## fig3d - total number of studies investigating each behavioural assay of behavioural_sociality
```{r}
fig3d <- bd %>%
...
## fig3e - total number of studies investigating each behavioural assay of behavioural_foraging
```{r}
fig3e <- bd %>%
...
## fig3f - total number of studies investigating each behavioural assay of anti predator behaviour
```{r}
fig3f <- bd %>%
...
## fig4a - heat map to show which behavours have been investigated in which pesticide target class
```{r}
...
fig4a <- ggplot(bd_pd_summary1, aes(x = pesticide_target_class, y = behavioural_class, fill = count)) +
...
## fig4b - heat map to show which behavours have been investigated in which pesticide chemical class
```{r}
...
# Create a heatmap using ggplot2
fig4b <- ggplot(bd_pd_summary_top5, aes(x = pesticide_chemical_class, y = behavioural_class, fill = count)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "#BFEFFF", high = "#0047AB") +
...
## fig4c - heat map to show which life stages of exposure have been exposed to each pesticide chemical class
```{r}
...
fig4c <- ggplot(sd_pd_summary1_top5, aes(x = pesticide_chemical_class, y = life_stage_exposure, fill = count)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "#BFEFFF", high = "#0047AB") +
...
## fig4d - heat map to show which life stages of exposure have been exposed to each target chemical class
```{r}
...
fig4d <- ggplot(sd_pd_summary2_top5, aes(x = pesticide_target_class, y = life_stage_exposure, fill = count)) +
  geom_tile(color = "white") +
  scale_fill_gradient(low = "#BFEFFF", high = "#0047AB") +
  geom_text(aes(label = ifelse(count > 0, count, "")), color = "black", size = 4) +
...
## fig5a - general bibliometric summaries
```{r}
...
fig5d <- networkPlot(NetMatrix_authors,  n = 50, 
...
# Create a network matrix of collaborations between continents
NetMatrix_continent <- biblioNetwork(bib_sco2, analysis = "collaboration", network = "countries", sep = ";")

# Convert the network matrix to a standard matrix
NetMatrix_continent <- as.matrix(NetMatrix_continent)

# Remove the lower triangle (as this is duplication of info)
NetMatrix_continent[lower.tri(NetMatrix_continent)] <- 0 

# Change column and row names to title case
colnames(NetMatrix_continent) <- str_to_title(colnames(NetMatrix_continent))
rownames(NetMatrix_continent) <- str_to_title(rownames(NetMatrix_continent))
...
#rownames(NetMatrix_continent)[rownames(NetMatrix_continent) =="United Kingdom"] <- "Europe"
...
# Create a chord diagram of the network matrix
fig5G <- chordDiagram(NetMatrix_continent, annotationTrack = "grid", preAllocateTracks = 1)
